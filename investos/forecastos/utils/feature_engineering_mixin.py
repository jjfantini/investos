...
class FeatureEngineeringMixin:
    @classmethod
    def apply_feature_engineering_logic(
        cls,
        df,
        config,
        features_key,
        ...
        calculate_with=None,
        global_logic_dict_key=None,
    ):
        for ft_name, ft in (
            (k, v)
            for k, v in config.get(features_key, {}).items()
            if not calculate_with or v.get("calculate_with") == calculate_with
        ):
            for formula_name, arg_li in ft.get(
                logic_dict_key, config.get(global_logic_dict_key, {})
            ).items():
    ...
    def apply_formula(cls, df, ft_name, formula_name, arg_li):
        ...
        if formula_method is None:
            raise ValueError(
                f"Formula method `{method_name}` not found on `{cls.__name__}`"
            )
    ...
    def apply_winsorize(cls, df: pd.DataFrame, ft_name: str, arg_li: list):
        ...
        df[ft_name] = df.groupby(group_by)[ft_name].transform(
            lambda x: x.clip(lower=x.quantile(lower_q), upper=x.quantile(upper_q))
        )
        return df
    ...
    def apply_map_ticker_to_fsym(cls, df: pd.DataFrame, ft_name: str, arg_li: list):
        ticker_df = fos.get_feature_df("79f61521-381e-4606-8020-7a9bc3130260").rename(
    ...
    @classmethod
    def apply_multiply_time_window_by_scalar(
        cls, df: pd.DataFrame, ft_name: str, arg_li: list
    ):
