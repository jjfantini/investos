    ...
    def create_feature_df(cls, config={}, base_df=None, merge_asof=True):
        ...
        for ft_name, ft in config.get("features", []).items():
            ...
            for formula_name, arg_li in ft.get(
                ...
                config.get("feature_adjustments_pre_join", {}),
            ).items():
            ...
            if merge_asof:
                ...
                df = pd.merge_asof(
                    df, tmp_ft_df, on="datetime", by="id", direction="backward"
                )
            else:
        ...
        df = cls.apply_feature_engineering_logic(
            df, config, "features", logic_dict_key="adjustments"
        )
        df = cls.apply_feature_engineering_logic(
            df,
            config,
        ...
        df = cls.apply_feature_engineering_logic(
            ...
            global_logic_dict_key="feature_normalization",
        )
        df = cls.apply_feature_engineering_logic(
            df,
            config,
