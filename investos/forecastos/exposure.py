...
from investos.forecastos.utils.feature_engineering_mixin import FeatureEngineeringMixin
...
class Exposure(Readable, FeatureEngineeringMixin):
    ...
    def get_df(cls, id):
        ...
        if res.ok:
            ...
            df[["poll_start_at", "poll_end_at", "filing_date"]] = (
                df[["poll_start_at", "poll_end_at", "filing_date"]]
                .apply(pd.to_datetime)
                .apply(lambda col: col.dt.tz_localize(None))
            )
            return df
        else:
            print(res)
    ...
    def create_exposure_df(cls, config={}, base_df=None, merge_asof=True):
        ...
        for exposure_name, exposure in config.get("exposures", []).items():
            ...
            tmp_exposure_df = cls.get_df(exposure["id"]).rename(
                columns={
            ...
            for formula_name, arg_li in exposure.get(
                "adjustments_pre_join",
                config.get("exposure_adjustments_pre_join", {}),
            ).items():
                tmp_exposure_df = cls.apply_formula(
                    tmp_exposure_df, exposure_name, formula_name, arg_li
                )
        ...
        df = cls.apply_feature_engineering_logic(
            df,
            config,
            "exposures_derived",
            logic_dict_key="formula",
            calculate_with="raw",
        )

        # Run adjustments on all (excl. normalized derived features)
        df = cls.apply_feature_engineering_logic(
            df, config, "exposures", logic_dict_key="adjustments"
        )
        df = cls.apply_feature_engineering_logic(
            df,
            config,
            "exposures_derived",
            logic_dict_key="adjustments",
            calculate_with="raw",
        )
        ...
        df = cls.apply_feature_engineering_logic(
            ...
            config,
            "exposures",
            logic_dict_key="normalization",
            global_logic_dict_key="exposure_normalization",
        )
        df = cls.apply_feature_engineering_logic(
            df,
            config,
        ...
        df = cls.apply_feature_engineering_logic(
            ...
            config,
            "exposures",
            logic_dict_key="adjustments_post_normalization",
        )
        df = cls.apply_feature_engineering_logic(
            df,
            config,
